...
interface AuthContextValue {
  ...
  updateProfile: (data: { name?: string; university?: string; bio?: string; avatarUrl?: string }) => Promise<void>;
  logout: () => Promise<void>;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateProfile = async (profileData: { name?: string; university?: string; bio?: string; avatarUrl?: string }) => {
    const t = localStorage.getItem(TOKEN_KEY);
    if (!t) throw new Error("غير مصرح");
    const currentUser = await apiFetch<AuthUser>("/auth/me", undefined, t);
    const data = await apiFetch<AuthUser>(`/users/${currentUser.id}`, {
      method: "PATCH",
      body: JSON.stringify(profileData),
    }, t);
    setUser(data);
  ...
  return (
    <AuthContext.Provider value={{ user, token, isLoading, sendOtp, verifyOtp, setupProfile, updateProfile, logout, refetchUser }}>
      {children}
